...
export default class MessageCreate extends Listener {
    ...
    async run(message) {
        const row = new MessageActionRow().addComponents(new MessageButton().setCustomId("FUNNY_ACTION_YES").setStyle("PRIMARY").setLabel("yes"), new MessageButton().setCustomId("FUNNY_ACTION_CANCEL").setStyle("DANGER").setLabel("no im a fucking pussy"));
        if (message.content === "i have no choice") {
            if (envParseArray('DEV_IDS').includes(message.author.id)) {
                const collector = (await message.channel.send({
